...
function transform_to_own_delta(manufacturer,model, mm2m, T, user_mode)
%save current path
path=pwd;
...
for i=0:robot.DOF
    file_i_base=sprintf('/01_Original(ascii)/link%d_base.stl', i);
    ...
    case 'save'
        save_dir_ascii = '/02_after_transform(ascii)';
        save_dir_binary = '/03_after_transform(binary)';
        file_i_bin = sprintf(['/link%d_bin.stl'], i);
        stlwrite([robot.path save_dir_ascii file_i], fout, V, 'mode', 'ascii');
        stlwrite([robot.path save_dir_binary file_i_bin], fout, V, 'mode', 'binary');
    otherwise
        error("transform_to_own_delta(): user_mode wrong.")
    end
...
disp(['Successfully save STLs to "', save_dir_ascii, '" and "', save_dir_binary, '"'])
end
